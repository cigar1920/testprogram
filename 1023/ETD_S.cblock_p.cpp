...
int main(int argc, char *argv[]) {
  ...
  double Dvac;
  Dvac = 1.0e-6 *
         exp((0.0 - EMvac) / (KB * T)); // m^2/s。1.0e-6*exp(-EMvac/(KB*T))m^2/s
  ...
  double Alpha0, Beta0;
  Beta0 = Alpha0 =
      pow((48.0 * PI * PI / Vat / Vat), 1.0 / 3.0) * Dvac; /*30  97  98*/
  double temp_var_for_const_0, temp_var_for_const_1, temp_var_for_const_2,
      temp_var_for_const_3, temp_var_for_const_4, temp_var_for_const_5,
  ...
  for (i = 0; i < N; i++) {
    ...
    Beta[i] = Beta0 * pow((i + 1.0), 1.0 / 3.0); /*98  103  104*/
    double temp_var_for_const_10, temp_var_for_const_11, temp_var_for_const_12,
        ...
        temp_var_for_const_28, temp_var_for_const_29;
    ...
    if (i == 0) {
      ...
      poly3[0] =
          (0.0 - 4.0) + 3.0 * dt * (1.0 + k) - dt * dt * (1.0 + k) * (1.0 + k) +
          exp((0.0 - dt) * (1.0 + k)) * (4 + dt * (1.0 + k)); /*104  117  118*/
      double temp_var_for_const_30, temp_var_for_const_31,
          ...
          temp_var_for_const_53, temp_var_for_const_54, temp_var_for_const_55,
      ...
      double temp_var_for_tac_22, temp_var_for_tac_23, temp_var_for_tac_24,
          ...
          temp_var_for_tac_82, temp_var_for_tac_83;
      ...
      double temp_var_for_const_70, temp_var_for_const_71,
          ...
          temp_var_for_const_90, temp_var_for_const_91;
      double temp_var_for_tac_84, temp_var_for_tac_85, temp_var_for_tac_86,
          ...
          temp_var_for_tac_144, temp_var_for_tac_145;
  ...
  while (t < TIME) {
    ...
    for (i = 0; i < N; i++) {
      if (i == 0) {
        alpha[i] = 0.0;
        beta[i] = 0.0; /*149  151  152*/
        double temp_var_for_const_92, temp_var_for_const_93;
        ...
        alpha[i] = Alpha[i] * C[i];
        beta[i] = Beta[i] * C[i] * C[0]; /*152  154  155*/
    ...
    for (i = 1; i <= N - 2; i++) {
      Fc[i] = Beta[i - 1] * C[i - 1] * C[0] - Beta[i] * C[i] * C[0] +
              Alpha[i + 1] * C[i + 1] + k * C[i]; /*163  165  166*/
    ...
    for (i = 0; i < N; i++) {
      a[i] = 0.0;
      a[i] = phi1[i] * C[i] + phi2[i] * Fc[i]; /*175  177  178*/
      double temp_var_for_tac_159, temp_var_for_tac_160, temp_var_for_tac_161,
          temp_var_for_tac_162;
      ...
      if (i == 0) {
        alpha_a[i] = 0.0;
        beta_a[i] = 0.0; /*178  180  181*/
        double temp_var_for_const_96, temp_var_for_const_97;
        ...
        beta_a[i] = Beta[i] * a[i] * a[0]; /*181  183  184*/
      ...
      sumbeta_a += beta_a[i]; /*184  186  187*/
    }
    Fa[0] = -2 * Beta[0] * a[0] * a[0] - sumbeta_a + sumalpha_a +
            Alpha[1] * a[1] + (1.0 + k) * a[0]; /*187  189  190*/
    ...
    for (i = 1; i <= N - 2; i++) {
      Fa[i] = Beta[i - 1] * a[i - 1] * a[0] - Beta[i] * a[i] * a[0] +
              Alpha[i + 1] * a[i + 1] + k * a[i]; /*190  192  193*/
    ...
    for (i = 0; i < N; i++) {
      b[i] = phi1[i] * C[i] + phi2[i] * Fa[i]; /*200  201  202*/
      double temp_var_for_tac_176, temp_var_for_tac_177, temp_var_for_tac_178,
      ...
      if (i == 0) {
        ...
        beta_b[i] = Beta[i] * b[i] * b[0]; /*205  207  208*/
      ...
      sumbeta_b += beta_b[i]; /*208  210  211*/
    }
    Fb[0] = -2 * Beta[0] * b[0] * b[0] - sumbeta_b + sumalpha_b +
            Alpha[1] * b[1] + (1.0 + k) * b[0]; /*211  213  214*/
    ...
    for (i = 1; i <= N - 2; i++) {
      Fb[i] = Beta[i - 1] * b[i - 1] * b[0] - Beta[i] * b[i] * b[0] +
              Alpha[i + 1] * b[i + 1] + k * b[i]; /*214  216  217*/
    ...
    for (i = 0; i < N; i++) {
      d[i] = phi1[i] * a[i] + phi2[i] * (2 * Fb[i] - Fc[i]); /*224  225  226*/
      double temp_var_for_const_103;
      double temp_var_for_tac_193, temp_var_for_tac_194, temp_var_for_tac_195,
          temp_var_for_tac_196, temp_var_for_tac_197, temp_var_for_tac_198;
      ...
      if (i == 0) {
        ...
        beta_d[i] = Beta[i] * d[i] * d[0]; /*229  231  232*/
      ...
      sumbeta_d += beta_d[i]; /*232  234  235*/
    }
    Fd[0] = -2 * Beta[0] * d[0] * d[0] - sumbeta_d + sumalpha_d +
            Alpha[1] * d[1] + (1.0 + k) * d[0]; /*235  237  238*/
    ...
    for (i = 1; i <= N - 2; i++) {
      Fd[i] = Beta[i - 1] * d[i - 1] * d[0] - Beta[i] * d[i] * d[0] +
              Alpha[i + 1] * d[i + 1] + k * d[i]; /*238  240  241*/
    ...
    for (i = 0; i < N; i++) {
      ...
      temp_var_for_ext_1 =
          ...
          pow(dt, -2) * L3[i] * (poly1[i] * Fc[i] + poly2[i] * (Fa[i] + Fb[i]) +
                                 poly3[i] * Fd[i]); /*248  253  256*/
      double temp_var_for_tac_212, temp_var_for_tac_213, temp_var_for_tac_214,
          temp_var_for_tac_215, temp_var_for_tac_216, temp_var_for_tac_217,
          temp_var_for_tac_218, temp_var_for_tac_219, temp_var_for_tac_220,
          temp_var_for_tac_221, temp_var_for_tac_222;
    ...
    sumalpha = sumalpha_a = sumalpha_b = sumalpha_d = 0.0; /*265  266  269*/
