...
double __ieee754_acosh(double x) {
  ...
  if (hx < IC(0x3ff00000)) { /* x < 1 */
    return (x - x) / (x - x);
  } else if (hx >= IC(0x41b00000)) { /* x > 2**28 */
...
double __acosh(double x) {
  ...
  temp_var_for_ext_0 = __ieee754_acosh(x);
  double temp_var_for_callexp_0 =
      __ieee754_acosh(getFVbyShadow<double>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignD({(Addr)&temp_var_for_ext_0}, (Addr)&temp_var_for_callexp_0,
          "/home/shijia/Public/testprogram/e_acosh.c_e.c:70:22");
