...
float __ieee754_acoshf(float x) {
  ...
  if (hx < IC(0x3f800000)) { /* x < 1 */
    return (x - x) / (x - x);
  } else if (hx >= IC(0x4d800000)) { /* x > 2**28 */
  ...
  } else if (hx > IC(0x40000000)) { /* 2**28 > x > 2 */
    t = x * x;
...
float __acoshf(float x) {
  ...
  temp_var_for_ext_0 = __ieee754_acoshf(x);
  float temp_var_for_callexp_0 =
      __ieee754_acoshf(getFVbyShadow<float>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignF({(Addr)&temp_var_for_ext_0}, (Addr)&temp_var_for_callexp_0,
          "/home/shijia/Public/testprogram/e_acoshf.c_e.c:57:22");
