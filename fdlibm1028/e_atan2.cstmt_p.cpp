...
double __atan2(double y, double x) /* wrapper atan2 */
{
  if (_LIB_VERSION == _SVID_ && x == 0.0 && y == 0.0)
    return __kernel_standard(y, x, HUGE_VAL,
                             KMATHERR_ATAN2); /* atan2(+-0,+-0) */

  double temp_var_for_ext_0;
  temp_var_for_ext_0 = __ieee754_atan2(y, x);
  double temp_var_for_callexp_0 = __ieee754_atan2(
      getFVbyShadow<double>((Addr)&y), getFVbyShadow<double>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignD({(Addr)&temp_var_for_ext_0}, (Addr)&temp_var_for_callexp_0,
          "/home/shijia/Public/testprogram/e_atan2.c_e.c:154:22");
