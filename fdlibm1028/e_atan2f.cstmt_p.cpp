...
float __ieee754_atan2f(float y, float x) {
  ...
  static const float pi_lo = (0.0 - 8.7422776573e-08);
...
float __atan2f(float y, float x) {
  if (x == 0.0f && y == 0.0f && _LIB_VERSION == _SVID_)
    return __kernel_standard_f(y, x, HUGE_VALF,
                               KMATHERRF_ATAN2); /* atan2(+-0,+-0) */

  float temp_var_for_ext_0;
  temp_var_for_ext_0 = __ieee754_atan2f(y, x);
  float temp_var_for_callexp_0 = __ieee754_atan2f(
      getFVbyShadow<float>((Addr)&y), getFVbyShadow<float>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignF({(Addr)&temp_var_for_ext_0}, (Addr)&temp_var_for_callexp_0,
          "/home/shijia/Public/testprogram/e_atan2f.c_e.c:124:22");
