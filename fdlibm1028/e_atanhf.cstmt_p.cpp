...
float __ieee754_atanhf(float x) {
  ...
  AssignF({(Addr)&temp_var_for_ext_0}, (Addr) & (-(t)),
          "/home/shijia/Public/testprogram/e_atanhf.c_e.c:100:22");
...
float __atanhf(float x) {
  if (isgreaterequal(__ieee754_fabsf(x), 1.0F) && _LIB_VERSION != _IEEE_)
    return __kernel_standard_f(x, x, HUGE_VALF,
                               __ieee754_fabsf(x) > 1.0F
                                   ? KMATHERRF_ATANH_PLUSONE /* atanh(|x|>1) */
                                   : KMATHERRF_ATANH_ONE);   /* atanh(|x|==1) */

  float temp_var_for_ext_1;
  temp_var_for_ext_1 = __ieee754_atanhf(x);
  float temp_var_for_callexp_0 =
      __ieee754_atanhf(getFVbyShadow<float>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignF({(Addr)&temp_var_for_ext_1}, (Addr)&temp_var_for_callexp_0,
          "/home/shijia/Public/testprogram/e_atanhf.c_e.c:119:22");
