...
long double __ieee754_atanhl(long double x) {
  ...
  AssignLd({(Addr)&temp_var_for_ext_0}, (Addr)&t,
           "/home/shijia/Public/testprogram/e_atanhl.c_e.c:75:22");
...
long double __atanhl(long double x) {
  if (isgreaterequal(__ieee754_fabsl(x), 1.0) && _LIB_VERSION != _IEEE_)
    return __kernel_standard_l(x, x, HUGE_VALL,
                               __ieee754_fabsl(x) > 1.0
                                   ? KMATHERRL_ATANH_PLUSONE /* atanh(|x|>1) */
                                   : KMATHERRL_ATANH_ONE);   /* atanh(|x|==1) */

  long double temp_var_for_ext_1;
  temp_var_for_ext_1 = __ieee754_atanhl(x);
  long double temp_var_for_callexp_0 =
      __ieee754_atanhl(getFVbyShadow<long double>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignLd({(Addr)&temp_var_for_ext_1}, ,
           "/home/shijia/Public/testprogram/e_atanhl.c_e.c:92:22");
