...
double __ieee754_fmod(double x, double y) {
  ...
  static const double Zero[] = { 0.0, (0.0 - 0.0);
...
if ((hx | lx) == 0) /* return sign(x)*0 */
  return Zero[(uint32_t)sx >> 31];
while (hx < IC(0x00100000)) { /* normalize x */
  hx = hx + hx + (lx >> 31);
  lx = lx + lx;
  iy -= 1;
}
...
computeDSub((Addr)&temp_var_for_ext_0, {(Addr) &(temp_var_for_const_1 = 0.0),
                                        (Addr) &(temp_var_for_const_0 = 0.0)},
            "/home/shijia/Public/testprogram/e_fmod.c_e.c:154:22");
...
double __fmod(double x, double y) {
  ...
  temp_var_for_ext_1 = __ieee754_fmod(x, y);
  double temp_var_for_callexp_0 = __ieee754_fmod(
  ...
  AssignD({(Addr)&temp_var_for_ext_1}, (Addr)&temp_var_for_callexp_0,
          "/home/shijia/Public/testprogram/e_fmod.c_e.c:167:22");
