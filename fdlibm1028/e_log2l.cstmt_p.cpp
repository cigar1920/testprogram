...
long double __ieee754_log2l(long double x) {
  ...
  long double temp_var_for_const_0;AssignLd({(Addr)&y},(Addr)&(temp_var_for_const_0=__ieee754_frexpl(getFVbyShadow<int>((Addr)&x), ((&(e)))),"/home/shijia/Public/testprogram/e_log2l.c_e.c:35:5");
  if (y < M_SQRT1_2l) {
    y = 2.0L * y;
    e = e - 1;
  }
  ...
  if (islessequal(x, 0.0) && _LIB_VERSION != _IEEE_) {
    if (x == 0.0) {
      ...
      return __kernel_standard_l(x, x, -HUGE_VALL,
                                 KMATHERRL_LOG2_ZERO); /* log2(0) */
    } else {
      feraiseexcept(FE_INVALID);
      return __kernel_standard_l(x, x, __builtin_nanl(""),
                                 KMATHERRL_LOG2_MINUS); /* log2(x<0) */
    }
  }

  long double temp_var_for_ext_0;
  temp_var_for_ext_0 = __ieee754_log2l(x);
  long double temp_var_for_callexp_0 =
      __ieee754_log2l(getFVbyShadow<long double>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignLd({(Addr)&temp_var_for_ext_0}, ,
           "/home/shijia/Public/testprogram/e_log2l.c_e.c:61:22");
