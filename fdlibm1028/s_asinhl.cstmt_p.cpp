...
long double __ieee754_asinhl(long double x) {
  ...
  if (ix > 0x4020) { /* |x| > 2**34 */
    ...
    if (ix > 0x4000) { /* 2**34 > |x| > 2.0 */
      w = __ieee754_logl(2.0L * xa +
                         one / (__ieee754_sqrtl(xa * xa + one) + xa));
    } else { /* 2.0 > |x| > 2**-28 */
      t = xa * xa;
  ...
  AssignLd({(Addr)&temp_var_for_ext_0}, (Addr) & (-(w)),
           "/home/shijia/Public/testprogram/s_asinhl.c_e.c:70:22");
...
long double __asinhl(long double x) {
  ...
  temp_var_for_ext_1 = __ieee754_asinhl(x);
  long double temp_var_for_callexp_0 =
      __ieee754_asinhl(getFVbyShadow<long double>((Addr)&x));
  CallStackPop((Addr)&temp_var_for_callexp_0, getTop(temp_var_for_callexp_0));
  AssignLd({(Addr)&temp_var_for_ext_1}, (Addr)&temp_var_for_callexp_0,
           "/home/shijia/Public/testprogram/s_asinhl.c_e.c:80:22");
