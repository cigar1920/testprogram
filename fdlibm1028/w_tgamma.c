...
double __tgamma(double x)
{
	...
	if (!isfinite(y) && (isfinite(x) || isinf(x) < 0) && _LIB_VERSION != _IEEE_)
	{
		if (x == 0.0)
			{
return __kernel_standard(x, x, y, KMATHERR_TGAMMA_ZERO);
}
	/* tgamma pole */
		else {
if (__ieee754_floor(x) == x && x < 0.0)
			{
return __kernel_standard(x, x, y, KMATHERR_TGAMMA_MINUS);
}
	/* tgamma domain */
		else {
if (y == 0)
			{
return __kernel_standard(x, x, y, KMATHERR_TGAMMA_UNDERFLOW);
}
	/* tgamma underflow */
		else
			{
return __kernel_standard(x, x, y, KMATHERR_TGAMMA_OVERFLOW);
}
